...
{	
	...
	public class DamageOnTouch : CorgiMonoBehaviour 
	{
		...
		protected virtual void Awake()
		{
			...
			//231122
			Debug.Log("Damage Method Ready");
            Debug.Log(gameObject.name);

            var rx = new Regex("RetroMachineGunBullet-*", RegexOptions.Compiled);
            bool result = rx.IsMatch(gameObject.name);

			Debug.Log(result);

            if (rx.IsMatch(gameObject.name))
			{
                Debug.Log("Damage Method Complete");
                if (simdata.characterLevel ==0)
				{

				}
				if(simdata.characterLevel >= 1)
				{
                    MinDamageCaused = 10*simdata.characterLevel;
                    MaxDamageCaused = 10 * simdata.characterLevel;
            ...
            var isRetroLergeBullet = new Regex("RetroLargeBullet-*", RegexOptions.Compiled);
            if (isRetroLergeBullet.IsMatch(gameObject.name))
            {
                Debug.Log("Damage Method Complete");
                if (simdata.characterLevel == 0)
                {
                ...
                if (simdata.characterLevel >= 1)
                {
                    MinDamageCaused = 10 + 10 * simdata.characterLevel;
                    MaxDamageCaused = 10 + 10 * simdata.characterLevel;
            ...
            /*var isRetroComboSword = new Regex("RetroComboSword\\(Clone\\)DamageArea", RegexOptions.Compiled);
            if (isRetroComboSword.IsMatch(gameObject.name))
            {
                Debug.Log("RetroComboSword(Clone)DamageArea Damage Method Complete");
                if (simdata.characterLevel == 0)
                {
                ...
                if (simdata.characterLevel >= 1)
                {
                    MinDamageCaused = 20 + 10 * simdata.characterLevel;
                    MaxDamageCaused = 20 + 10 * simdata.characterLevel;
